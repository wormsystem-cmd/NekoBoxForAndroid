...
class MainActivity : ThemedActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Auto Fetch Online Configs
        fetchOnlineConfigs()
        
        // Spinner Setup for Online Configs
        binding.akiraConfigSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
            override fun onItemSelected(parent: AdapterView<*>?, view: android.view.View?, position: Int, id: Long) {
                if (position > 0) { // Skip the first "Select Server" item
                    val selectedConfig = onlineConfigs[position - 1]
                    runOnDefaultDispatcher {
                        try {
                            val profiles = parseProxies(selectedConfig.second)
                            profiles.forEach { profile ->
                                val proxyEntity = ProfileManager.createProfile(DataStore.selectedGroup, profile)
                                onMainDispatcher {
                                    cbSelectorUpdate(proxyEntity.id)
                                }
                            }
                        } catch (e: Exception) {
                            onMainDispatcher {
    ...
    private fun fetchOnlineConfigs() {
        runOnDefaultDispatcher {
            try {
                val url = java.net.URL("https://raw.githubusercontent.com/Akira/WormSystem/main/servers.json")
                ...
                val configsList = mutableListOf<Pair<String, String>>()
                val namesList = mutableListOf<String>()
                
                // Add default placeholder item
                namesList.add("Select Server")
                
                for (i in 0 until jsonArray.length()) {
                    val jsonObject = jsonArray.getJSONObject(i)
                    val name = jsonObject.getString("name")
                    val config = jsonObject.getString("config")
                    configsList.add(Pair(name, config))
                    namesList.add(name)
                }
                
                onlineConfigs = configsList
                ...
                onMainDispatcher {
                    snackbar("Failed to fetch online configs: ${e.message}").show()
                }
            }
        }
    }
